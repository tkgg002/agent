...
**Fix — Chiến lược: Accumulate highest offset, commit SAU flush:**

```go
// Track highest offset per partition — KHÔNG lưu toàn bộ messages vào memory
// Kafka offset là chuỗi liên tục: commit offset 100 = acknowledge 1-99
type topicPartition struct {
    Topic     string
    Partition int
}

// Chỉ cần lưu int64, tiết kiệm RAM đáng kể khi traffic lớn
pendingOffsets map[topicPartition]int64
```

**Flow mới:**
```
Loop: FetchMessage → processMessage (add to buffer) → track highest offset
                                                          ↓
Buffer đầy hoặc timer 5s → Flush to DB → thành công → CommitMessages(highest per partition)
...
2. Sau `processMessage` thành công: update `pendingOffsets[tp] = max(current, msg.Offset)`
3. Thêm callback vào `BatchBuffer.Flush()` — sau khi flush thành công, build `[]kafka.Message` từ `pendingOffsets` rồi gọi `reader.CommitMessages()`
4. Khi flush fail → không commit → messages sẽ được re-deliver
5. Clear `pendingOffsets` sau commit thành công

> [!CAUTION]
> **Rủi ro Rebalance:** Trong cửa sổ 5s chờ buffer đầy, nếu Kafka consumer group rebalance (scale up/down, worker chết), partition bị assign cho worker khác. Code hiện tại **KHÔNG có** `OnRevoked/OnAssigned` callback (chỉ có `RebalanceTimeout: 30s`). Cần:
> - `BatchBuffer.Flush()` phải tôn trọng `context.Done()` → flush ngay khi context bị cancel
> - Trước khi nhả partition (revoke), BẮT BUỘC flush buffer + commit pending offsets
> - Nếu flush fail khi revoke → log Error, chấp nhận re-delivery (at-least-once)

**Verification:** Unit test simulate crash giữa processMessage và flush → assert messages re-delivered. Test rebalance scenario → assert flush trước revoke.
...
+             // Cancel context để release mọi DB connections đang pending
+             cancel()
+         }
+     }()
      defer span.End()
...
> [!WARNING]
> **Rủi ro rò rỉ tài nguyên khi recover:** Goroutine gọi `h.svc.Run()` → bên trong `Run()` có `sync.WaitGroup` + `mu.Lock()` + DB transactions. Nếu panic giữa `Lock()` và `Unlock()` → deadlock. Nếu panic giữa DB TX → connection leak.
>
> **Mitigation:** Gọi `cancel()` trong recover block để cancel `bgCtx` → tất cả DB operations dùng `bgCtx` sẽ bị rollback. `bgCtx` đã được tạo ở line 207 với `defer cancel()`, nhưng khi panic xảy ra, defer chain có thể không chạy đúng thứ tự → gọi `cancel()` tường minh trong recover là an toàn nhất.
>
> **Lưu ý:** Mutex `mu` trong `Run()` thuộc scope local của goroutine con (trong `wg.Add/go func`), không phải goroutine handler → panic ở goroutine con sẽ crash goroutine con, WaitGroup sẽ hang → `bgCtx` timeout sẽ cancel → DB connections release. Recover ở handler goroutine chỉ bắt panic **trước/sau** `Run()`.
...
**Fix — switch type pattern (cover bẫy `float64` của `encoding/json`):**

> [!WARNING]
> **Bẫy Go:** Nếu payload đi qua `encoding/json` → `json.Unmarshal` vào `map[string]interface{}`, Go mặc định parse **mọi con số thành `float64`**, không phải `int64`. Dùng `comma-ok` check `.(int64)` sẽ trả `false` → **vô tình drop data hợp lệ**.
>
> Trong case hiện tại, `_gpay_id` được gán từ `deterministicGpayID()` (return `int64`) nên CHƯA bị. Nhưng defensive coding bắt buộc cover `float64` để tránh regression khi flow thay đổi.
...
+     gpayID = int64(v)
+ default:
+     t.logger.Warn("dedup: _gpay_id unexpected type, skipping",
...
+ case int64:  currentTs = v
+ case float64: currentTs = int64(v)
+ default: bestIndices[gpayID] = idx; continue
+ }
+ switch v := allRecords[storedIdx]["_source_ts"].(type) {
+ case int64:  storedTs = v
+ case float64: storedTs = int64(v)
+ default: bestIndices[gpayID] = idx; continue
+ }
```

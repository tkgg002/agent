...
- Lọc danh sách `reports` (phiên chưa xử lý) và `healedReports` (phiên đã xử lý) bằng filter:
  - Nếu `segment` là `'source_shadow'`, chỉ hiển thị các phiên đối soát có `r.segment === 'source_shadow'` hoặc `!r.segment` (ngầm định là chặng A).
  - Nếu `segment` là `'shadow_master'`, chỉ hiển thị các phiên có `r.segment === 'shadow_master'`.

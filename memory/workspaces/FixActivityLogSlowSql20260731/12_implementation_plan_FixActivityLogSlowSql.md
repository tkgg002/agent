...
> [!IMPORTANT]
> - Thay đổi này hoàn toàn không làm đổi cấu trúc dữ liệu trả về (wire contract) của các API `/api/activity-log` và `/api/activity-log/stats`.
> - Cần ý kiến đồng ý (APPROVE) từ User để giao Muscle tiến hành triển khai file Migration SQL và refactor mã nguồn Go.
...
### Automated Tests
- Chạy biên dịch kiểm tra syntax Go: `go build ./...` tại repo `cdc-cms-service`.

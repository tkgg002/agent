# Plan: Tier 2 XOR-Hash Reconciliation Analysis / Kế Hoạch Phân Tích Đối Soát Tier 2 XOR-Hash

This plan outlines the steps for auditing and analyzing the Tier 2 window-based XOR-hash reconciliation flow inside `centralized-data-service`.
...
6. **ID & Timestamp Mapping Audit**: Verify that the primary key ID and timestamp fields conform to the new shadow registration schema (e.g., `_id` as PK, `lastUpdatedAt` as Timestamp Field).
7. **Output & Healing Flow Audit**: Investigate what results are returned after Tier 2, where they are stored, and how the subsequent heal process runs to fix the drifts.
8. **CMS-Web Flow Audit**: Analyze how CMS-Web reads reports and triggers the heal action.
9. **Heal Execution & State Transition Audit**: Investigate the case reported by the user (why report 7 had mismatched but did not heal them instantly, why report 8 transitioned to "healed" with null stale_ids/mismatched).
10. **Heal Path Audit (FetchAndWriteByIDs vs NATS Debezium Signal)**: Verify whether the active heal path goes through direct MongoDB write (FetchAndWriteByIDs) or sends a Debezium signal to "cdc.cmd.debezium-signal".
11. **False Mismatched Detection Audit**: Inspect why the Tier 2 reconciliation logic detected 3 mismatched IDs when there were actually none (false mismatched/drift detection).
12. **DDL Timezone Core Mapping Audit**: Investigate the core DDL generation and type mapping logic to find why timestamp fields are mapped to TIMESTAMP instead of TIMESTAMPTZ in the shadow database.
13. **Heal Routing Inconsistency Audit**: Audit heal routing logic to find why window drift (Branch 1) always sends Debezium signals and doesn't fallback to the direct write path (Branch 2) like Safety Net when Debezium is disabled.
14. **FE/BE Co-Design & Routing Redesign specs**: Design the FE modal controls (radio buttons for Window vs Full-diff, dynamic from/to inputs, max 30-day validation) and modify the BE routing logic to execute based on the mode param, replacing the Debezium NATS signal with direct FetchAndWriteByIDs in Window mode.
...
### Các Nhiệm Vụ Phân Tích Đề Xuất
1. **Xác định file mã nguồn**: Định vị các file đang hoạt động trong thư mục `internal/service/recon/`.
2. **Phân tích luồng kiểm soát ReconCore**: Phân tích hàm `RunTier2` in `recon_tier_a.go` để hiểu cách tính toán cửa sổ thời gian, cách so sánh XOR-hash và cách drill-down xử lý sai lệch.

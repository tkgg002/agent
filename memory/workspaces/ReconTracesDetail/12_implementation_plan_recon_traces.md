...
2. Trong `RunHashWindowCheck` và `RunHashWindowCheckB`, inject cờ bypass qua helper:
   ...
   ```
3. Trong `ReconSourceAgent.HashWindow` và `ReconDestAgent.HashWindow`, kiểm tra qua helper:
   ```go
...
3. **Kiểm thử tích hợp bổ sung:**
   - **Kịch bản có Drift thật:** Sửa đổi thủ công dữ liệu ở destination DB để tạo drift, xác nhận hệ thống tự động fallback về window check (672 windows) và sinh span drill down chuẩn xác.
   - **Kiểm tra rủi ro broken link qua NATS:** Kiểm tra SigNoz để đảm bảo cây Trace con nằm đúng dưới span cha `cdc.recon.check` được truyền từ NATS handler.
